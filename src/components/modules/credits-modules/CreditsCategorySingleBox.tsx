...
interface CardProps { id: number; title: string; desc: string; isShowVamapp: boolean; path: string; }
interface CreditsCategorySingleBoxProps { card: CardProps; index: number; selectedCard: number; setSelectedCard: (index: number) => void; isShowVamapp: boolean; }



const CreditsCategorySingleBox: React.FC<CreditsCategorySingleBoxProps> = ({ card, index, selectedCard, setSelectedCard, isShowVamapp }) => {
      return (
            <Card key={card.id} sx={{ width: "100%", height: "25vh", margin: "10px 1.5px", borderRadius: "8px", boxShadow: 2, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", border: "none" }}>
                  <CardActionArea
                  ...
                  >
                        <CardContent sx={{ height: "100%", display: "flex", flexDirection: "column", gap: "8px", padding: "8px", justifyContent: "space-between", alignItems: "center" }}>
                              ...
                              <div className="flex flex-col items-stretch justify-between w-full ">
                                    ...
                                    <div className="flex w-full items-center justify-end  rounded-md mt-4">
                                          <Button component={Link} color="primary" variant="contained" to={card.path} sx={{ textDecoration: "none", padding: "4px 10px", letterSpacing: "-0.5px", fontWeight: "600", fontFamily: "Vazir" }}>
                                                درخواست وام
                                          </Button>
                                    </div>
                              </div>
