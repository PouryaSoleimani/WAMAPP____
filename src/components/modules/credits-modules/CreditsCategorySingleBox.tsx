...
const CreditsCategorySingleBox: React.FC<CreditsCategorySingleBoxProps> = ({ card, index, selectedCard, setSelectedCard }) => {
      ...
      return (
            <Card key={card.id} 
                  sx={{ width: "100%", flex: 1, margin: "5px 1.5px", borderRadius: "8px", boxShadow: 2, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "stretch", border: "none" }}>
                  <CardActionArea
                  ...
                  >
                        <CardContent sx={{ height: "100%", display: "flex", flexDirection: "column", padding:"8px"}}>
                              <div className="flex flex-col items-start justify-start gap-2 basis-[90%]">
                                    ...
                                    <p className="text-sm text-start w-[60%] h-[60px] text-[var(--text-secondary)]">{card.desc}</p>
                              </div>
